...
export function debugTableAddresses(data: Uint8Array, param: IDefinitionParameter, calOffset: number = 0): void {
    const rows = param.rows || 1;
    const cols = param.cols || 1;
    const typeSize = DATA_TYPE_INFO[param.dataType].size;
...
export function debugFindDataOffset(data: Uint8Array, param: IDefinitionParameter, calOffset: number = 0): void {
    const cols = param.cols || 1;
    const typeSize = DATA_TYPE_INFO[param.dataType].size;
...
export function debugLayoutComparison(data: Uint8Array, param: IDefinitionParameter, calOffset: number = 0): void {
    const rows = param.rows || 1;
    const cols = param.cols || 1;
    const typeSize = DATA_TYPE_INFO[param.dataType].size;
    const dataOffset = param.dataOffset ?? 0;
